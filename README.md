...
jobs:
  ...
  test:
    ...
    steps:
      ...
      - name: Process dotnet test results and create a status check
        ...
        uses: Tech-Innovation/process-dotnet-test-results@v2.2.3
        with:
  ...
  update-deployment-board:
    ...
    if: always()
    steps:
      - uses: Tech-Innovation/workflow-conclusion@v2.2.2
        id: conclusion
        with:
          github-token: ${{ secrets.GITHUB_TOKEN }}
      ...
      - name: Update Deployment Board
        uses: Tech-Innovation/update-deployment-board@v1.5.1
        with:
          github-token: ${{ secrets.GITHUB_TOKEN}}
